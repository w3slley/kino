...
router.get("/getFavorites/:userId", (req, res) => {
  ...
  FavoriteMovie.find({ userId: userId }, (err, data) => {
    if (err) {
      res.send({
        status: "failed",
        message: "Error while retreiving favorite movies",
      });
    } else {
...
router.post("/addFavorite", (req, res) => {
  FavoriteMovie.find(
    { userId: req.body.userId, imdbID: req.body.imdbId },
    (err, data) => {
      if (err) {
        res.send({
          status: "failed",
          message: "Error while adding favorite movie",
        });
      } else if (data.length == 0) {
        let newFavoriteMovie = new FavoriteMovie({
        ...
        newFavoriteMovie.save((err) => {
          if (err) {
            res.send({
              status: "failed",
              message: "Error while saving favorite movie to database",
            });
          } else {
...
router.post("/removeFavorite", (req, res) => {
  FavoriteMovie.deleteOne({ imdbID: req.body.imdbID }, (err, data) => {
    if (err) {
      ...
      res.send({
        status: "failed",
        message: "Error while removing favorite movie",
      });
    } else {
      res.send({ status: "success" });
    }
...
router.get("/:imdbId", async (req, res) => {
  ...
  try {
    ...
    movieData["YoutubeId"] = await getYoutubeId(
      movieData.Title,
      movieData.Year,
    );
